...
function ensureColumn(table, column, definition){
  try { const cols=db.prepare(`PRAGMA table_info(${table})`).all(); if(!cols.some(c=>c.name===column)) db.exec(`ALTER TABLE ${table} ADD COLUMN ${column} ${definition}`); } catch(e) {}
}
db.exec(`
...
ensureColumn("users","role","TEXT DEFAULT 'player'");
ensureColumn("users","credit","REAL DEFAULT 0");
...
app.use(express.static(path.join(__dirname,"public"), {setHeaders:(res,file)=>{if(file.endsWith("admin.html")||file.endsWith("admin.js")||file.endsWith("admin.css")) res.setHeader("Cache-Control","no-store");}}));
...
app.get("/api/admin/me",requireAdmin,(req,res)=>res.json({ok:true,user:userOut(req.admin)}));
